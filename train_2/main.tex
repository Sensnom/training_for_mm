...
进气口、排气口和等效泄漏通道均采用准稳态可压缩孔口流\cite{blair1999engine}。对上游状态$(p_u,T_u)$和下游压力$p_d$，单向质量流率为
\begin{equation}
...
燃烧过程采用Wiebe函数\cite{ghojel2010wiebe}。令$z=(\varphi-\varphi_0)/\Delta\varphi_b$，在$0<z<1$内有
\begin{equation}
...
壁面换热采用压力、温度和转速的简化幂律关联，而不是完整Woschni模型\cite{woschni1967heat}；泄漏采用$0.03\,\mathrm{mm^2}$代表性等效孔口。该泄漏面积是相似机型敏感性研究中的工程先验，不解释为目标机实测间隙。机械损失通过摩擦平均有效压力表示\cite{heywood2018ice}：
\begin{equation}
...
参考鲁棒优化中以不确定集合评估最坏情形的思想\cite{bental2002robust}，设置基准情景$(1.00,1.00,0^\circ)$及四种组合扰动$(0.96,1.10,+5^\circ)$、$(1.04,0.90,-5^\circ)$、$(0.98,1.05,-5^\circ)$和$(1.02,0.95,+5^\circ)$，三元组依次表示燃烧效率倍率、摩擦倍率和重叠角偏差。对各情景的四项指标作截断线性效用变换，以权重$(0.40,0.25,0.20,0.15)$合成评分，并定义
\begin{equation}
