...
本目录提供论文问题一、二的完整可复现求解链：

> 严格读取附件 → 自动生成合法班型 → 日内整数覆盖 → 解析确定招聘规模 → 最大流安排休息日 → 员工—班次确定性映射 → 员工级反算校验 → 完整 MILP 独立复核。

问题三不在本次重构范围内。`solve_q3_test.py`、`solve_repair_43.py`、`solve_q2.py` 和 `build_verified_results.py` 是旧版实验脚本，标记为 legacy，不会被主入口调用，其中的绝对路径和 OR-Tools 写法不代表当前主链。
